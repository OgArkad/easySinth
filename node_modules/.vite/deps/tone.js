	...
	(function(global, factory) {
		...
		var truncateValueCurve = function truncateValueCurve(values, originalDuration, targetDuration) {
			...
			for (var i = 0; i < truncatedLength; i += 1) {
				var theoreticIndex = i / (truncatedLength - 1) * targetDuration / originalDuration * (length - 1);
	...
	nativeAudioNode.disconnect = ((disconnect) => {
		return (destinationOrOutput, output, input) => {
			...
			if (destinationOrOutput === void 0) connections.clear();
				...
				if (output === void 0) connections.delete(destinationOrOutput);
				else {
...
var createNativeStereoPannerNodeFakerFactory = (createNativeChannelMergerNode, createNativeChannelSplitterNode, createNativeGainNode, createNativeWaveShaperNode, createNotSupportedError, monitorConnections) => {
	...
	const buildInternalGraphForStereo = (nativeContext, inputGainNode, panGainNode, channelMergerNode) => {
		...
		for (let i = 0; i < CURVE_SIZE; i += 1) if (i > centerIndex) {
			const x = (i - centerIndex) / 8192 * HALF_PI;
			leftInputForLeftOutputWaveShaperCurve[i] = Math.cos(x);
			leftInputForRightOutputWaveShaperCurve[i] = Math.sin(x);
			rightInputForLeftOutputWaveShaperCurve[i] = 0;
			rightInputForRightOutputWaveShaperCurve[i] = 1;
		} else {
			const x = i / 8192 * HALF_PI;
...
var createTestAudioBufferCopyChannelMethodsSubarraySupport = (nativeOfflineAudioContextConstructor) => {
	return () => {
		...
		const source = /* @__PURE__ */ new Float32Array(2);
		try {
	...
	fromType(type) {
		...
		switch (this.defaultUnits) {
			...
			case "hz":
				...
				break;
			case "midi": this._val = type.toMidi();
		}
		return this;
	}
	/**
	...
	*/
	get numberOfInputs() {
		if (isDefined(this.input)) {
			if (isAudioParam(this.input) || this.input instanceof Param) return 1;
			...
			this._state.forEachBetween(startTime, endTime, (e) => {
				switch (e.state) {
					...
					case "stopped":
						...
						break;
					case "paused": this.emit("pause", e.time);
				}
			});
...
var EnvelopeCurves = (() => {
	...
	let k;
	const cosineCurve = [];
	for (i = 0; i < curveLen; i++) cosineCurve[i] = Math.sin(i / 127 * (Math.PI / 2));
	const rippleCurve = [];
	const rippleCurveFreq = 6.4;
	for (i = 0; i < 127; i++) {
		k = i / 127;
		const sineWave = Math.sin(k * (Math.PI * 2) * rippleCurveFreq - Math.PI / 2) + 1;
		rippleCurve[i] = sineWave / 10 + k * .83;
	}
	rippleCurve[127] = 1;
	const stairsCurve = [];
	const steps = 5;
	for (i = 0; i < curveLen; i++) stairsCurve[i] = Math.ceil(i / 127 * steps) / steps;
	const sineCurve = [];
...
var BiquadFilter = class BiquadFilter extends ToneAudioNode {
	...
	*/
	getFrequencyResponse(len = 128) {
		...
		for (let i = 0; i < len; i++) {
			const freq = Math.pow(i / len, 2) * 19980 + 20;
			freqValues[i] = freq;
		}
		...
		return new Proxy(array, {
			...
			set: (target, property, value) => {
				if (isString(property) && isFinite(parseInt(property, 10))) {
					if (isArray(value)) target[property] = this._createSequence(value);
